    ...
    <div>
        <div class="header">
            ...
            <div class="header-right">
                <van-field v-model="searchValue" shape="round" placeholder="请输入用户名" />
            </div>
        </div>
        <!-- 轮播图 -->
        <div class="my-swiper">
            <van-swipe :autoplay="3000" height="170px" lazy-render>
                <van-swipe-item v-for="image in homeData.slides" :key="image.id">
                    <van-image
                        :src="image.pic_image_url"
                        radius="5px"
                        />
                </van-swipe-item>
            </van-swipe>
        ...
        <!-- 快捷入口   layout布局将屏幕宽度分为24份-->
     
     <div class="banner">
        <van-row justify="space-between">
            <van-col span="11" v-for="(item,index) in homeData.nav2s" :key="index" @click="goOrderTwo(index)">
                <van-image
...
onMounted(async ()=>{
    const {data}=await proxy.$api.getIndex()
    data.data.now=dayjs(data.data.now).format('YYYY-MM-DD HH:mm:ss')
